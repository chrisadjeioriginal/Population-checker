...
let countryListOpened = false;
let listHolder = null;

function showCountries() {
  if (countryListOpened === false) {
    listHolder = document.createElement("div");
    listHolder.style.position = "absolute";
    listHolder.style.left = "0px";
    listHolder.style.top = "-505px";
    // listHolder.style.transform = "translateX(-50%)";
    listHolder.style.display = "flex";
    listHolder.style.width = "300px";
    listHolder.style.height = "500px";
    listHolder.style.overflow = "scroll";
    listHolder.style.backgroundColor = "gray";
    ...
    iso_codes.forEach((item) => {
      const newItem = document.createElement("li");
      newItem.textContent = item["countryLabel"];
      ...
      newItem.style.border = "0.5px solid black";
      newItem.addEventListener("mouseover", () => {
        newItem.style.backgroundColor = "rgb(33, 146, 163)";
      });
      ...
      newItem.addEventListener("click", (e) => {
        ...
        searchArea.value = dict1[countryName];
      });
    ...
    browseButton.appendChild(listHolder);

    document.body.addEventListener("click", (e) => {
      if (countryListOpened == true) {
        listHolder.remove();
        countryListOpened = false;
      }
    });

    countryListOpened = true;
  }
}

async function getPopulation(country) {
  const url = `/population?country=${country}`;
...
async function getFlag(country) {
  const url = `/flag?country=${country}`;
  const Response = await fetch(url);
...
iso_codes.forEach((item) => {
  dict1[item["countryLabel"]] = item["alpha2"];
});
